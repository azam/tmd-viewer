...
use actix_web::{
    get, http::header::CONTENT_TYPE, middleware, post, web, App, HttpResponse, HttpServer,
...
struct AppState {
    ...
    scanner_count_limit: RwLock<i32>,
    time_offset: f32,
}
...
fn state(data: web::Data<AppState>) -> AppStateExternal {
    AppStateExternal {
        ...
        scanner_count_limit: *data.scanner_count_limit.read().unwrap(),
    }
}
        ...
        Some(value) => {
            ...
            *data.data_dir.write().unwrap() = value.to_string();
            ...
            let config = AppConfig {
                ...
                scanner_count_limit: Some(*data.scanner_count_limit.read().unwrap()),
            };
...
async fn generate_thumbnails_service(data: web::Data<AppState>) -> impl Responder {
    if *data.scanner_count.read().unwrap() >= *data.scanner_count_limit.read().unwrap() {
...
async fn main() -> std::io::Result<()> {
    ...
    let app_state = web::Data::new(AppState {
        ...
        scanner_count_limit: RwLock::new(scanner_count_limit),
